...
vector<Rect> detectFaces(Mat img_gray){
    ...
    faces_cascade.load("/Users/ruogulu/Desktop/Study/Algorithm/Practice/CV/FaceDetection/Object_Detection_img/Object_Detection_img/haarcascade_frontalface_default.xml"); //注意路径问题，出于方便我放到与main.cpp相同的目录下，
                                                                    //文件夹与xxcode目录下都要有xml文件，，并且检查
                                                                    //拼写是否正确，否则报错OpenCV(3.4.1) Error:
                                                                    //Assertion failed (!empty()) in
    if( !faces_cascade.load("/Users/ruogulu/Desktop/Study/Algorithm/Practice/CV/FaceDetection/Object_Detection_img/Object_Detection_img/haarcascade_frontalface_default.xml"))
        printf("--(!)Error loading face cascade in detectFaces\n");
    else
        printf("--(!)Success loading face cascade in detectFaces\n");
...
void saveFaces(Mat img,Mat img_gray){
    ...
    for(size_t i=0; i<faces.size();i++){
        stringstream buffer;
        buffer<<i;
//        string saveName = "/Users/ruogulu/Desktop/Pictures/KnownFaces/"+ buffer.str() + ".jpeg";
        string saveName = "/Users/ruogulu/Desktop/Results/"+ buffer.str() + ".jpeg";
        Rect roi = faces[i];
...
void detectDrawEyes(Mat img,Mat img_gray){
    ...
    for(size_t i=0; i<faces.size();i++){
        ...
        eyes_cascade.load("/usr/local/Cellar/opencv/3.4.1_3/share/OpenCV/haarcascades/haarcascade_eye.xml");
        if(!eyes_cascade.load("/usr/local/Cellar/opencv/3.4.1_3/share/OpenCV/haarcascades/haarcascade_eye.xml"))
            printf("--(!)Error loading eye cascade in detectDrawEyes\n");
        else
            printf("--(!)Success loading eye cascade in detectDrawEyes\n");
        vector<Rect> eyes;
...
int main(){
    ...
    Mat img = imread("/Users/ruogulu/Desktop/mask10.jpeg");
    Mat img_gray;
